...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Determine Prefix
    $prefix = '';
    if ($item_category == 'Main Dish') {
        $prefix = 'MD';
    } elseif ($item_category == 'Side Snacks') {
        $prefix = 'SS';
    } elseif ($item_category == 'Drinks') {
        $prefix = 'D';
    } else {
        $prefix = 'ITEM';
    }

    // Query existing IDs with this prefix to find the max suffix number
    $id_query = "SELECT item_id FROM Menu WHERE item_id LIKE '$prefix%'";
    $id_result = mysqli_query($conn, $id_query);
    $max_num = 0;
    if ($id_result) {
        while ($row = mysqli_fetch_assoc($id_result)) {
            $existing_id = $row['item_id'];
            $num_str = substr($existing_id, strlen($prefix));
    ...
    $insert_query = "INSERT INTO Menu (item_id, item_name, item_category, item_price, item_description, status) 
                    VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($insert_query);

    // Bind the parameters
    $stmt->bind_param("sssdss", $item_id, $item_name, $item_category, $item_price, $item_description, $status);

    // Execute the query
    if ($stmt->execute()) {
        $message = "Item created successfully. Allocated ID: " . $item_id;
...
require_once '../posBackend/checkIfLoggedIn.php'; echo $bgColor; ?>;
        }
        .alert-success i {
...
<body>
    <div class="card <?php
require_once '../posBackend/checkIfLoggedIn.php'; echo $cardClass; ?>" style="display: none;">
        <div style="border-radius: 200px; height: 200px; width: 200px; background: #F8FAF5; margin: 0 auto;">
            <?php
require_once '../posBackend/checkIfLoggedIn.php'; if ($iconClass === 'fa-check-circle'): ?>
                <i class="checkmark">✓</i>
            <?php
...
require_once '../posBackend/checkIfLoggedIn.php'; endif; ?>
        ...
        <h1><?php
require_once '../posBackend/checkIfLoggedIn.php'; echo ($cardClass === 'alert-success') ? 'Success' : 'Error'; ?></h1>
        <p><?php
