...
if (isset($_SESSION['logged_account_id'])) {
    ...
    if ($admin_check_res && $admin_check_row = mysqli_fetch_assoc($admin_check_res)) {
        if ($admin_check_row['role'] == 'Admin') {
            header("Location: ../menuCrud/toggleStatus.php?id=" . urlencode($item_id) . "&status=" . urlencode($status));
            exit();
        }
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // User-provided input
    $provided_account_id = trim($_POST['admin_id'] ?? ''); // 112233
    $provided_password = trim($_POST['password'] ?? ''); // 12345
...
<body>
    <div class="login-container">
        ...
        <h5 class="panel-subtitle">Admin Credentials needed to Toggle Status</h5>
        
        <form action="" method="post" autocomplete="off">
            <div class="form-group mb-4">
                <label>Admin Id</label>
                <input type="number" name="admin_id" class="form-control" placeholder="Enter Admin ID" required autocomplete="off">
            </div>
            ...
            <div class="form-group mt-2">
                <button class="btn btn-submit mb-3" type="submit" name="submit" value="submit">Toggle Status</button>
                <a class="btn btn-cancel d-block text-center" href="../panel/menu-panel.php" >Cancel</a>
            </div>
        </form>
    </div>
