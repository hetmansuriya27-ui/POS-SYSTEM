...
// Header
$pdf->Cell(0, 10, "X Hotel", 0, 1, 'C');
$pdf->SetFont('Arial', '', 12);
$pdf->Cell(0, 10, 'Receipt ID: ' . $bill_data['bill_id'], 0, 1, 'L');
$pdf->Cell(0, 10, 'Date: ' . $bill_data['bill_time'], 0, 1, 'L');

// Display table, staff, and order source details on the same row
$rowText = 'Table ID: ' . $bill_data['table_id'] . ' | Staff ID: ' . $bill_data['staff_id'] . ' | Source: ' . $bill_data['order_source'];
$pdf->Cell(0, 10, $rowText, 0, 1, 'L');

if (!empty($bill_data['non_member_name']) && !empty($bill_data['non_member_mobile'])) {
    $nonMemberText = 'Customer Name: ' . $bill_data['non_member_name'] . ' | Mobile: ' . $bill_data['non_member_mobile'];
    $pdf->Cell(0, 10, $nonMemberText, 0, 1, 'L');
}
