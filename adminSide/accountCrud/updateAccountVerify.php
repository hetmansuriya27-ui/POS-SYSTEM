...
if (isset($_SESSION['logged_account_id'])) {
    ...
    if ($admin_check_res && $admin_check_row = mysqli_fetch_assoc($admin_check_res)) {
        if ($admin_check_row['role'] == 'Admin') {
            header("Location: ../accountCrud/updateAccount.php?id=" . $member_id);
            exit();
        }
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($uniqueString == "112233112233@Xhotel") {
        header("Location: ../accountCrud/updateAccount.php?id=" . $account_id);
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // User-provided input
    $provided_account_id = $_POST['admin_id']; // 112233
    $provided_password = $_POST['password']; // 12345
...
<body>
    <div class="login-container">
        ...
        <h5 class="panel-subtitle">Admin Credentials needed to Edit Account</h5>
        ...
        <form action="" method="post">
            ...
            <div class="form-group mt-2">
                <button class="btn btn-submit mb-3" type="submit" name="submit" value="submit">Edit Account</button>
